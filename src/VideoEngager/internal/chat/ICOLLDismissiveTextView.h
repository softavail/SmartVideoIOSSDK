...
/**
 *  The delegate of a `ICOLLDismissiveTextView` object must adopt the `ICOLLDismissiveTextViewDelegate` protocol.
 */
@protocol ICOLLDismissiveTextViewDelegate <NSObject>
...
/**
 *  An instance of `ICOLLDismissiveTextView` is a means for displaying a text view that is contained as a subview of the keyboard's `inputAccessoryView` and responds to a pan gesture to dismiss the keyboard and end editing.
 */
@interface ICOLLDismissiveTextView : UITextView

/**
 *  The object that acts as the delegate of the receiving text view.
 */
@property (weak, nonatomic) id<ICOLLDismissiveTextViewDelegate> keyboardDelegate;
